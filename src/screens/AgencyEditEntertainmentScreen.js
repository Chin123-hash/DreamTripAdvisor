...
const AgencyEditEntertainmentScreen = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                if (id) {
                    const data = await getEntertainmentById(id);
                    if (data) {
                        setFormData({
                            title: data.title || '',
                            ...
                            ticketPrice: data.ticketPrice ? data.ticketPrice.toString() : '0',
    ...
    const handleSave = async () => {
        ...
        setSubmitting(true);
        try {
            const ticket = parseFloat(formData.ticketPrice) || 0;
            const transport = parseFloat(formData.transportCost) || 0;
            const total = ticket + transport;

            const updatedData = {
                ...formData,
                estimatedTotalExpenses: total,
            };
...
const styles = StyleSheet.create({
    ...
    // --- Google Places Styles Override ---
    googleInput: {
        backgroundColor: '#F5F7FA',
        borderRadius: 10,
        paddingHorizontal: 15,
        paddingVertical: 10, // Slightly less padding for height match
        fontSize: 16,
        color: '#333',
        height: 50,
    },
...
export default AgencyEditEntertainmentScreen;